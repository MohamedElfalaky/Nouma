...
class HomeCubit extends Cubit<HomeStates> {
  ...
  void getmyroom() {
    ...
    DioHelper.getdata(url: getroom, token: token).then((value) {
      ...
      emit(GetMyRoomErrorStates(error.toString()));
    });
  }
  ...
  // List<dynamic> inroomListtimeseriesglobal = [];
  ...
  void getroomuser({@required id}) {
    ...
    DioHelper.getdata(url: 'rooms/$id/users', token: token).then((value) {
      ...
      //   userList.add(InRoomUserModelModel.fromJson(element.data()));
      // });

      // print(inroomList);
      // inroomList = value.data;
      // value.data.forEach((element) {
  ...
  void roomUsersNow({
    ...
    // RoomModel model = RoomModel(userNow: roomUsersNow, roomID: roomid);
    // if (ismuted = null) {
    //   ismuted = true;
    // }
    var documentReference = FirebaseFirestore.instance
  ...
  void followingroom() {
    ...
    DioHelper.getdata(url: following, token: token).then((value) {
  ...
  void usersfollowingroom({@required id}) {
    ...
    DioHelper.getdata(url: 'followers/room/$id', token: token).then((value) {
      userFollowingRoomModel = UserFollowingRoomModel.fromJson(value.data);

      // for (int i = 0;
      //     i < userFollowingRoomModel.data[i].followers.length;
      //     i++) {
      //   allUserFollowRoom = userFollowingRoomModel.data[i].followers.length;
      // }

      // for (int i = 0;
      //     i < userFollowingRoomModel.data[i].followers.length;
      //     i++) {
      //   if (specialId == userFollowingRoomModel.data[i].followers[i].userId) {
      //     iFollow = true;
      //   } else {
      //     iFollow = false;
      //   }
      ...
      //   if ('888888' == userFollowingRoomModel.data.first.followers[i].userId) {
      //     iFollow = true;
  ...
  void myroomcheckroom() {
    ...
    DioHelper.getdata(url: myroom, token: token).then((value) {
