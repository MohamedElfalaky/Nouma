...
class _DetailsScreenState extends State<DetailsScreen> {
  ...
  readLocal() async {
    // apiid = PreferencesServices.getString(ID_KEY);
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<HomeCubit, HomeStates>(listener: (context, state) {
        ...
        if (state is InroomSuccessStates) {
          ...
          // for(int i=0;i<HomeCubit.get(context).roomUserModel.data.length;i++){
        ...
        //     notificationModel = NotificationModel.fromJson(event.data);
        //     // isHaveFrame = null;
        //     // if (model.data.user.entry == 'ther is no entry') {
        //     // } else {
        //     new Future.delayed(Duration.zero, () {
        ...
        // if (state is JoinmicSuccessStates) {
        //   FirebaseMessaging.onMessage.listen((event) {
        //     print(event.data.toString());
        //     NotificationModel notificationModel;
        //     notificationModel = NotificationModel.fromJson(event.data);
        ...
        return ConditionalBuilder(
          ...
          builder: (context) => getdata(
              ...
              HomeCubit.get(context).roomUserModel.message,
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                                      ...
                                      child: Container(
                                        child: Row(
                                          children: [
                                            ...
                                            // StreamBuilder(stream: _firestoreInstance.collection('rooms').doc(widget.roomId).snapshots(), builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot) {
                                            //    var messeage = snapshot.get('userNow');
                                            //   return Text()
                                            //  },
                                            //   )
                          ...
                          // Row(
                          //   children: [
                          //     roomMicslayout(micnum = 1),
                          //     roomMicslayout(micnum = 2),
                          //     roomMicslayout(micnum = 3),
                          //     roomMicslayout(micnum = 4),
                          //     roomMicslayout(micnum = 5),
                          //   ],
                          // ),

                          // Row(
                          //   children: [
                          //     roomMicslayout(micnum = 6),
                          //     roomMicslayout(micnum = 7),
                          //     roomMicslayout(micnum = 8),
                          //     roomMicslayout(micnum = 9),
                          //     roomMicslayout(micnum = 10),
                          //   ],
                          // ),
